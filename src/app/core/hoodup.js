(function (root, factory) {
  ...
  if (typeof exports === 'object') {
    ...
    module.exports = factory(backbone, hoodie, jquery);

  } else if (typeof define === 'function' && define.amd) {

    define(['backbone', 'hoodie', 'jquery'], factory);

  } else {
    // Browser globals
    root.hoodie = factory(root.Backbone, root.Hoodie, root.jQuery);
  }

}(this, function (Backbone, Hoodie, jQuery) {

  'use strict';

  var hoodie, sync, defaults, connect, log, enable_logging;
  ...
  function fetch_associations( modelOrCollection, modelConstructor, options ){
    ...
    log('Sync %o: checking associations(%o)', modelConstructor.prototype.type, rels);
      ...
      if(!state){
        ...
        switch (rel.type) {
            ...
            log('removing duplicate ids %o',_( state[type] ).pluck( 'id' ));
  					if ( state[ type ] ) {
              rel_ids = _(rel_ids).difference( _( state[type] ).pluck( 'id' ) );
            }
            where['id'] = rel_ids;
  					
  				break;
  				case 'many': //has many via foreign object references
  					rel_ids = _( models ).pluck( 'id' ); //fetch all that reference these objects
            ...
            log('removing duplicate ids %o',_( state[type] ).pluck( rel.id ));
            if ( state[ type ] ) {
              rel_ids = _(rel_ids).difference( _( state[type] ).pluck( rel.id ) );
            }
            where[ 'id' ] = rel_ids;
          break;
  			}
    ...
    log('Waiting for promises to resolve', promises);// promises[0].state());
    if (promises.length) {
