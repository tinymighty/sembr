...
define(['ractive', 'backbone'], function (Ractive, Backbone) {
	...
	Ractive.adaptors.Backbone = {
      ...
      //console.log('Filtering model %o, %o', object, (object instanceof Backbone.Model || object instanceof Backbone.Collection) );
	...
	BackboneModelWrapper = function ( ractive, model, keypath, prefix ) {
		var wrapper = this;
    //console.log('Wrapping model %o', model);
		this.value = model;
	...
	BackboneModelWrapper.prototype = {
		get: function () {
      return this.value.toJSON({include_associations:true});
		},
	...
	BackboneCollectionWrapper = function ( ractive, collection, keypath, prefix ) {
		var wrapper = this;
console.log('Wrapping collection %o', collection);
